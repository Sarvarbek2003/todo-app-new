...
app.post('/todos', (req, res) => {
    ...
    try {
        ...
        if( !(typeof title === 'string') || title.length < 20 ) return res.json({message:'Invalid title'})
        if( !(typeof text === 'string') || text.length < 100 ) return res.json({message: 'Invalid text'})
